...
import {OPCODE_FTSO_CURRENT_PRICE_USD, OPCODE_FTSO_CURRENT_PRICE_PAIR, OPCODE_SLFR_CURRENT_EXCHANGE_RATE} from "./FlareFtsoExtern.sol";
...
import {SUB_PARSER_WORD_FTSO_CURRENT_PRICE_USD, SUB_PARSER_WORD_FTSO_CURRENT_PRICE_PAIR, SUB_PARSER_WORD_SFLR_EXCHANGE_RATE, SUB_PARSER_WORD_PARSERS_LENGTH} from "../lib/parse/LibFlareFtsoSubParser.sol";
import {OPERAND_HANDLER_FUNCTION_POINTERS as SUB_PARSER_OPERAND_HANDLERS, SUB_PARSER_WORD_PARSERS, PARSE_META as SUB_PARSER_PARSE_META} from "../generated/FlareFtsoWords.pointers.sol";
...
abstract contract FlareFtsoSubParser is BaseRainterpreterSubParserNPE2 {
    ...
    /// @inheritdoc BaseRainterpreterSubParserNPE2
    function subParserOperandHandlers()
        internal
        pure
        override
        returns (bytes memory)
    ...
    function buildOperandHandlerFunctionPointers()
        ...
        returns (bytes memory)
    {
        function(uint256[] memory) internal pure returns (Operand)[]
            memory fs = new function(uint256[] memory)
                internal
                pure
                returns (Operand)[](SUB_PARSER_WORD_PARSERS_LENGTH);
        fs[SUB_PARSER_WORD_FTSO_CURRENT_PRICE_USD] = LibParseOperand
            .handleOperandDisallowed;
        fs[SUB_PARSER_WORD_FTSO_CURRENT_PRICE_PAIR] = LibParseOperand
            .handleOperandDisallowed;
        fs[SUB_PARSER_WORD_SFLR_EXCHANGE_RATE] = LibParseOperand
            .handleOperandDisallowed;
    ...
    function buildLiteralParserFunctionPointers()
        external
        pure
        returns (bytes memory)
    {
    ...
    function buildSubParserWordParsers() external pure returns (bytes memory) {
        function(uint256, uint256, Operand)
            ...
            memory fs = new function(uint256, uint256, Operand)
                internal
                view
                returns (bool, bytes memory, uint256[] memory)[](
        ...
        ] = ftsoCurrentPriceUsdSubParser;
        fs[
            SUB_PARSER_WORD_FTSO_CURRENT_PRICE_PAIR
        ] = ftsoCurrentPricePairSubParser;
        fs[
            SUB_PARSER_WORD_SFLR_EXCHANGE_RATE
        ] = sFlrCurrentExchangeRateSubParser;
    ...
    function ftsoCurrentPriceUsdSubParser(
        uint256 constantsHeight,
        uint256 ioByte,
        Operand operand
    ) internal view returns (bool, bytes memory, uint256[] memory) {
    ...
    //slither-disable-next-line dead-code
    function ftsoCurrentPricePairSubParser(
        uint256 constantsHeight,
        uint256 ioByte,
        Operand operand
    ) internal view returns (bool, bytes memory, uint256[] memory) {
    ...
    /// address and index of the current pair price opcode index in the extern.
    //slither-disable-next-line dead-code
    function sFlrCurrentExchangeRateSubParser(
        uint256 constantsHeight,
        uint256 ioByte,
        Operand operand
    ) internal view returns (bool, bytes memory, uint256[] memory) {
